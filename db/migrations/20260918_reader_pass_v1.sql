...
create table if not exists paperback_claims (
  id uuid primary key default gen_random_uuid(),
  reader_pass_id uuid not null references reader_passes(id) on delete cascade,
  product_slug text not null,
  edition text not null,
  order_reference_hash text not null unique,
  activation_hash text not null,
  claim_ip_hash text,
  created_at timestamptz not null default now()
);
create index if not exists paperback_claims_pass_idx on paperback_claims(reader_pass_id, created_at desc);
create index if not exists paperback_claims_product_idx on paperback_claims(product_slug, edition, created_at desc);
