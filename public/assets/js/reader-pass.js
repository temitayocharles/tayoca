(()=> {
  ...
  if(paperbackEnabled){
    ...
    pf.addEventListener("submit",async e=>{
      ...
      const fd=new FormData(pf),activation_code=fd.get("activation_code"),order_reference=String(fd.get("order_reference")||"").trim(),reader_pass=String(fd.get("reader_pass")||"").trim();
      try{
        const r=await fetch("/api/reader-pass/paperback-claim",{method:"POST",headers:{"content-type":"application/json"},credentials:"same-origin",body:JSON.stringify({activation_code,order_reference,product,edition,...(reader_pass?{reader_pass}:{})})});
        const d=await r.json().catch(()=>({}));
        if(!r.ok){pm.textContent=r.status===429?"Too many activation attempts. Try again later.":d.error==="paperback_activation_unavailable"?"Paperback activation is not enabled for this edition yet.":d.error==="invalid_order_reference"?"Enter the Amazon order number in its 123-1234567-1234567 format.":d.error==="order_already_claimed"?"That Amazon order has already been used to activate companion access.":d.error==="activation_limit"?"Too many paperback activations were attempted for this title from this network. Try again later or contact support.":d.error==="paperback_not_verified"?"That paperback activation code does not match this edition.":"Paperback activation could not be completed.";return;}
