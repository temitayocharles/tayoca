(function(){
  ...
  if(header){var links=header.classList.contains('site-header')?header.querySelector('nav'):header.querySelector('.nav-links');if(links&&!header.querySelector('.nav-toggle')){var toggle=document.createElement('button');toggle.type='button';toggle.className='nav-toggle';toggle.setAttribute('aria-expanded','false');toggle.setAttribute('aria-label','Open navigation');toggle.textContent='Menu';if(header.classList.contains('site-header'))header.insertBefore(toggle,links);else links.parentNode.insertBefore(toggle,links);toggle.addEventListener('click',function(){var open=header.classList.toggle('nav-open');toggle.setAttribute('aria-expanded',String(open));toggle.textContent=open?'Close':'Menu'});}}
