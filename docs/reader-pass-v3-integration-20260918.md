...
The printed activation code is not the long-lived credential. It is combined with a one-time Amazon order reference to deter casual sharing before a unique Reader Pass can be minted or attached. The Reader Pass remains the credential used for subsequent access.
...
- the browser submits no email or shipping address; paperback activation additionally requires the Amazon order number, which is normalized, HMAC-hashed server-side, and never stored raw;
- invalid codes or malformed order references fail closed without exposing the configured digest;
- an Amazon order reference can activate companion access only once across the system;
- successful title activation is additionally limited per network over a rolling 30-day window to reduce bulk pass minting from a shared code;
...
- the printed activation code plus unique order reference is intentionally treated as a stronger anti-sharing possession/purchase signal, but not as cryptographic verification against Amazon because KDP exposes no per-order validation API to this service.
