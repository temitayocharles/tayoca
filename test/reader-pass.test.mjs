...
test("paperback order reference is required and hashed before persistence",async()=>{const {readFile}=await import("node:fs/promises");const s=await readFile(new URL("../api/reader-pass/paperback-claim.js",import.meta.url),"utf8");assert.match(s,/validAmazonOrder/);assert.match(s,/hashReference\("amazon-order:"\+order\)/);assert.match(s,/order_already_claimed/);assert.match(s,/paperback_claims/);});

test("Gumroad claim UI does not request Amazon order while paperback activation does",async()=>{const {readFile}=await import("node:fs/promises");const s=await readFile(new URL("../public/assets/js/reader-pass.js",import.meta.url),"utf8");const g=s.slice(s.indexOf("Bought this edition on Gumroad?"),s.indexOf("if(paperbackEnabled)"));const p=s.slice(s.indexOf("if(paperbackEnabled)"));assert.equal(g.includes('name="order_reference"'),false);assert.equal(p.includes('name="order_reference"'),true);});
